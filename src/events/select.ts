...
export default class SelectHandler extends Event {
	constructor() { super("Select", "interactionCreate"); }
	...
	exec(interaction: discord.Interaction) {
		...
		try {
			...
			selectLogger.error(`Select ran by ${interaction.user.tag} (${interaction.user.id}) in ${interaction.guild?.name ?? "Not in guild"} (${interaction.guild?.id ?? "N/A"})`);
		}
	}
}