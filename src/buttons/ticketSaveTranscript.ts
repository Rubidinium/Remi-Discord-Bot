		...
		try {
			...
			interaction.message.components.forEach(row => {
				row.components.forEach(component => {
					if (component.customId == "ticketSaveTranscript") component.setDisabled();
				});
			});

			await (interaction.message as Message).edit({ components: interaction.message.components as MessageActionRow[] });
			...
			try {
				const user = await interaction.client.users.fetch(channel.name.split("-")[1]);
				user.send(`Here is your ticket transcript:\n${url}`).catch(() => {
					interaction.followUp(`${user} was unable to be notified of the transcript. They most likely have DMs disabled.`);
				});
				if (user != interaction.user) {
					interaction.followUp({ content: "You have been notified of the transcript.", ephemeral: true });
