 ...
 */
export default function generateTranscript(messages, channel, opts = { returnBuffer: false, fileName: "transcript.html" }) {
	...
	for (const message of (Array.from(messages.values())).sort((a, b) => a.createdTimestamp - b.createdTimestamp)) {
		...
		if (message.reference?.messageId) {
			...
			const author = referencedMessage?.author ?? staticS.DummyUser;

			reference.innerHTML =
				`<img class="chatlog__reference-avatar" src="${author.avatarURL() ?? staticS.defaultPFP}" alt="Avatar" loading="lazy">
		...
		// message author pfp
		const author = message.author ?? staticS.DummyUser;
		...
		authorAvatar.src = author.avatarURL() ?? staticS.defaultPFP;
	...
	content = purify(content)
		...
		.replace(/```(.+?)```/gs, code => {
			if (!replyStyle) {
				...
				if (staticS.LanguageAliases[language])
					language = staticS.LanguageAliases[language];
